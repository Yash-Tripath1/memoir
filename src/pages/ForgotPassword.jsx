...
export default function ForgotPassword() {
  ...
  const handleSubmitEmail = (e) => {
    ...
    // For this MVP, we just proceed to step 2.
    setStep(2);
  };
    ...
    <div className="min-h-screen flex items-center justify-center p-4" style={{ backgroundColor: 'var(--bg-primary)' }}>
      <div className="w-full max-w-md">
        ...
        <div className="card p-8">
          <div className="mb-6">
            <Link to="/login" className="flex items-center gap-1 text-sm text-memoir-400 hover:text-memoir-600 transition-colors">
              <ArrowLeft size={16} /> Back to login
            </Link>
          </div>

          <h2 className="text-xl font-semibold text-memoir-800 mb-2">Reset Password</h2>
          <p className="text-sm text-memoir-400 mb-6">
            {step === 1
          ...
          {success ? (
            ...
            <form onSubmit={handleResetPassword} className="space-y-4">
              <div>
                <label className="block text-sm font-medium text-memoir-600 mb-1">New Password</label>
                <input
                  type="password"
                  value={newPassword}
